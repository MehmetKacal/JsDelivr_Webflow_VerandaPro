let gegevensValidationAttempted = false; //Global flag gegevens tab
...
openTab(tabList[0]);
  ...
  //visible feedback when the user clicks “Next” without filling required fields on Gegevens page.
  $(we_tabs_next_button).on("click", function (e) {
    const activeTab = $(".w--tab-active").attr("data-w-tab");

    // Only validate if we're on the "Gegevens" tab
    if (activeTab !== "Gegevens") return;

    gegevensValidationAttempted = true; // 🔓 allow validation now

    const $section = $(".w--tab-active").find('.personal-info-section');
    ...
    $requiredFields.each(function () {
      ...
      // Always clear old error visuals
      ...
      if (!value) {
        ...
        $input.after(`<div class="error-message">Dit veld is verplicht</div>`);
    ...
    if (!allFilled) {
      e.preventDefault(); // Block tab change
    }
  });
  //this removes the error class when the user fills the field
  $('.personal-info-section').on("input change", "[required]", function () {
    if (!gegevensValidationAttempted) return; // 🔒 Don't show errors yet
...
function openTab(tab) {
  ...
  // Update "Prev" button text to "Home"
  var indexOfActiveTab = tabList.indexOf($(tab).attr("data-w-tab"));
  var attrPrevTab = $(tab).attr("prev-tab");
  if (indexOfActiveTab <= 0 || !attrPrevTab) {
    $(we_tabs_prev_button).text("Home");
  } else {
    $(we_tabs_prev_button).text("Terug"); // back to original label
  }
}
...
$("[name=products]").parent("label.w-radio").on("click", function () {
  ...
  //Function to enable next tab on form page when all required input is filled. Only do this in the tab Gegevens
  function enableNextTabWhenAllFilled(sectionSelector, nextTab, currentTabName = "Gegevens") {
    ...
    $requiredFields.on("input change", function () {
      ...
      // Only run logic if current tab is "Gegevens"
      ...
      if (allFilled) {
        const we_indexOfActiveTab = tabList.indexOf(activeTab);
        const we_indexOfNextTab = we_indexOfActiveTab + 1;
        const we_indexOfPrevTab = we_indexOfActiveTab - 1;
        const we_prevTab = tabList[we_indexOfPrevTab];
        const we_amountOfTabs = tabList.length;
        ...
        $(we_tabs_next_button).addClass(we_tabs_active_class);
        $(".w--tab-active").attr("next-tab", nextTab);
        $('[data-w-tab="' + nextTab + '"]').attr("prev-tab", activeTab);
      } else {
        $(we_tabs_next_button).removeClass(we_tabs_active_class);
      }
    });
...
//Next tab based on filling all required input
$(function () {
  enableNextTabWhenAllFilled('.personal-info-section', 'summary', 'Gegevens'); //Gegevens >> summary ; when filling input fields within .personal-info-section
});
